...
- `scripts/` — installer logic. Each top-level step has its own script:
  ...
  - `dotfiles.sh` — installs `~/.zshrc`, `~/.gitconfig`, `~/.tmux.conf`, and `~/.Codex/*` from `dotfiles/`.
...
- `dotfiles/` — source files copied into `$HOME`. `Codex/` holds Codex settings (separate `settings.json` and `settings-devbox.json`) plus `AGENTS.md` and `skills/`. `git/` has separate `gitconfig` and `gitconfig-devbox`.
- `configs/` — editor settings for `cursor/` and `vscode/`.

## Conventions

- Devbox variant: `--devbox` selects `brew-devbox.sh`, `gitconfig-devbox`, and `settings-devbox.json` (Codex in bypass-permissions mode).
