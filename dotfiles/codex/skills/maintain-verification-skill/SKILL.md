...
**Platform note.** On Codex, the parallel per-feature source readers below map to `spawn_agent` fan-out, and the project-local skill lives under Codex's skills location, not `.claude/skills/`. See [`codex-tools.md`](../poteto-mode/references/codex-tools.md).
