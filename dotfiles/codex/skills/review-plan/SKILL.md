...
You are reviewing the implementation plan provided by the user in the current prompt. If the prompt does not include the full plan text, ask for it before reviewing.

## Instructions

Spawn **six subagents in parallel** using Codex subagents when subagent delegation is available. Each subagent receives the full plan text as its prompt context. Each subagent has access to the codebase and should use fast search and targeted file reads to verify claims in the plan.
