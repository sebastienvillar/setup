...
For each actionable comment (or small group of related comments on the same file), spawn a subagent via the Agent tool to:
...
- Always end every reply with `_Commented by Claude_` on its own line so reviewers can tell the response came from an agent.
