...
argument-hint: [PR number, URL, or branch — defaults to the current branch]
...
If an argument was given, use it (`gh pr view <arg>`). Otherwise default to the current branch's PR:
...
For a large or wide-ranging diff, spawn parallel subagents (Agent tool) split by dimension or by file group, each returning concrete findings, then consolidate. For a small diff, review directly.
